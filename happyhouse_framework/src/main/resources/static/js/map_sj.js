...
function checkCondition(data,obj) {
    ...
    for (var i = 0; i < Object.keys(obj).length; i++) {
    	...
    	var category=Object.keys(obj)[i];
    	console.log(`obj= ${obj}`);
    	...
    	$.ajax({
    		url: "https://apis.openapi.sk.com/tmap/routes/pedestrian?"+ $.param ({"version" : 1,'startX': targetLon,
			...
			success: function (response) {
				...
				// 시간, 거리 데이터 추가
				aroundStr+=`${Math.round(time)}분, ${distance}m]<br>`;
			},
      		error: function (error){
      			console.log(error);
      		}
		...
		console.log(`Object.values(obj)[i]= ${obj[category]}`);
			
    	if(time>obj[category]) return false;
	}
