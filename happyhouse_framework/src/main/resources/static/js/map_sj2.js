...
function checkCondition(data,arr) {
    ...
    for (var i = 0; i < arr.length; i++) {
		// 가장 가까운 곳 poi 찾기 (경도 위도 빼내기)
    	var category=arr[i]["point"];
    	var transportation=arr[i]["transportation"];
    	var cdtTime=arr[i]["distance"];
    	console.log(`category= ${category}`);
    	
	    $.ajax({
	      	url: 'https://apis.openapi.sk.com/tmap/pois/search/around',
	      	headers: {Accept: 'application/json', appKey: 'l7xxfdc75c1509a74ecdba02bf5e024ee9d5'},
	      	async: false,
	      	data: {'version':1, 'centerLat': lat, 'centerLon': lng,'categories':category,'sort':'distance'},
	        	type: 'Get',
	        	dataType: 'json',
	        	success: function (response) {
	          		targetLat= response['searchPoiInfo']['pois']['poi'][0].frontLat;
	          		targetLon= response['searchPoiInfo']['pois']['poi'][0].frontLon;
	          		if(i!=0)aroundStr+=`<hr>`;
	          		aroundStr +=`<b>${response['searchPoiInfo']['pois']['poi'][0].name}</b> <br>[`;
	          	},
	      		error: function (error){
	      			console.log(error);
	      		}
	       	});
	    // 얻은 경도 위도로 도보 소요 시간 찾기, 저장한거보다 작으면 false를 반환
    	
	    console.log(`targetLon: ${targetLon}, targetLat: ${targetLat}, lng: ${lng}, lat: ${lat}`);
    	var time,distance;
    	
    	var url;
    	if (transportation=="도보") url='	https://apis.openapi.sk.com/tmap/routes/pedestrian';
    	else url="https://apis.openapi.sk.com/tmap/routes";
    	
    	$.ajax({
    		url: url+"?"+ $.param ({"version" : 1,'startX': targetLon,
  			  'startY': targetLat,'endX':lng,'endY':lat,'startName':'home','endName':'station'}),
    		headers: {
			...
			success: function (response) {
				...
				// 시간, 거리 데이터 추가
				aroundStr+=`${Math.round(time)}분 (${transportation}), ${distance}m]<br>`;
			},
      		error: function (error){
      			console.log(error);
      		}
		...
		console.log(`time= ${time},cdtTime= ${cdtTime}`);
			
    	if(time>cdtTime) return false;
	}
